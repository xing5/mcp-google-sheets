...
def get_sheet_formulas(spreadsheet_id: str,
    ...
    formulas = formula_response.get('values', [])
    return formulas
...
def batch_update_cells(spreadsheet_id: str,
    ...
    batch_response = sheets_service.spreadsheets().values().batchUpdate(
        spreadsheetId=spreadsheet_id,
        body=batch_body
    ).execute()
...
def rename_sheet(spreadsheet: str,
    ...
    # Execute the request
    rename_response = sheets_service.spreadsheets().batchUpdate(
...
def get_multiple_sheet_data(queries: List[Dict[str, str]],
    ...
    for query in queries:
        ...
        try:
            ...
            values = range_response.get('values', [])
        ...
        try:
            ...
            for sheet in spreadsheet.get('sheets', []):
                ...
                try:
                    ...
                    summary_response = sheets_service.spreadsheets().values().get(
                        spreadsheetId=spreadsheet_id,
                        range=range_to_get
                    ).execute()
...
def create_sheet(spreadsheet_id: str,
    ...
    # Execute the request
    create_sheet_response = sheets_service.spreadsheets().batchUpdate(
        spreadsheetId=spreadsheet_id,
        body=request_body
    ).execute()
...
def batch_update(spreadsheet_id: str,
    ...
    batch_update_result = sheets_service.spreadsheets().batchUpdate(
        spreadsheetId=spreadsheet_id,
        body=request_body
    ).execute()
